 <section class="banner-section">
            <div class="main-slider-carousel owl-carousel owl-theme">
                <div class="slide" style="background-image: url(images/1.png); background-size: cover; overflow:hidden; background-repeat: no-repeat;">
                    <div class="color-layer-one"></div>
                    <div class="auto-container">
                        <div class="row clearfix">
                            <div class="content-column col-lg-12 col-md-12 col-sm-12" style="margin-bottom: 100px;">
                                <div class="inner-column">
                                    <div class="title">We Are Dr Baba Group HERBAL LIFE CARE PVT LTD</div>
                                    <h1>Build a Healthier Lifestyle with <span>Dr Baba Group HERBAL LIFE CARE PVT LTD</span></h1>
                                    <div class="text">
                                        We provide premium organic wellness products, nutritional supplements, and plant-based formulations that support immunity, energy, fitness, and overall wellbeing.
                                    </div>
                                    <div class="btns-box">
                                        <a href="clog/signup.php" class="theme-btn btn-style-one">
                                            <span class="txt">Join Organic Journey</span>
                                        </a>
                                        <a href="about-us.html" class="theme-btn btn-style-two">
                                            <span class="txt">Know More</span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
